...
import (
	...
	"github.com/Hoverhuang-er/azbdb/internal"
	"github.com/Hoverhuang-er/azbdb/internal/sql"
	"github.com/Hoverhuang-er/azbdb/internal/sql/parse"
	sqlTypes "github.com/Hoverhuang-er/azbdb/internal/sql/types"
...
var ErrAZBConstraintPrimaryKey = errors.New("constraint: key not unique")
var ErrAZBConstraintUnique = errors.New("constraint: not unique")
...
func New(ctx context.Context, args []string) (*VirtualTable, error) {
	return newVirtualTable(ctx, args, AzureOptions{}, true)
}

func NewWithOptions(ctx context.Context, args []string, defaults AzureOptions) (*VirtualTable, error) {
	return newVirtualTable(ctx, args, defaults, false)
}

func newVirtualTable(ctx context.Context, args []string, defaults AzureOptions, registerGlobal bool) (*VirtualTable, error) {
	var err error
...
[entries_per_node=<N>,]            the number of rows to store per Azure Blob object (defaults to 4096)
[node_cache_entries=<N>,]          number of nodes to cache in memory (defaults to 0)
[readonly,]                        don't write to Azure Blob storage
azure_connection_string='...',      Azure Storage connection string
azure_container='mycontainer',      Azure Blob container name
[azure_account='account',]          defaults to AZURE_STORAGE_ACCOUNT
[azure_account_key='key',]          defaults to AZURE_STORAGE_KEY
[azure_endpoint='https://account.blob.core.windows.net/',]
                                   defaults to AZURE_STORAGE_BLOB_ENDPOINT or the account's public endpoint
[azure_prefix='/prefix',]           separate tables within a container`)
	}
	...
	for i := range args {
		...
		case "entries_per_node":
			...
			table.AzureOptions.EntriesPerNode = int(i)
		case "node_cache_entries":
			i, err := strconv.ParseInt(s[1], 32, 0)
			if err != nil {
				return nil, fmt.Errorf("arg: %w", err)
			}
			table.AzureOptions.NodeCacheEntries = int(i)
		case "readonly":
			table.AzureOptions.ReadOnly = true
		case "azure_connection_string":
			conn := internal.UnquoteAll(s[1])
			parsed, err := AzureOptionsFromConnectionString(conn)
			if err != nil {
				return nil, fmt.Errorf("azure_connection_string: %w", err)
			}
			table.AzureOptions.ConnectionString = parsed.ConnectionString
			if table.AzureOptions.Container == "" {
				table.AzureOptions.Container = parsed.Container
			}
			if table.AzureOptions.Prefix == "" {
				table.AzureOptions.Prefix = parsed.Prefix
				prefixSpecified = parsed.Prefix != ""
			}
			if parsed.ReadOnly {
				table.AzureOptions.ReadOnly = true
			}
		case "azure_container":
			table.AzureOptions.Container = internal.UnquoteAll(s[1])
		case "azure_account":
			table.AzureOptions.AccountName = internal.UnquoteAll(s[1])
		case "azure_account_key":
			table.AzureOptions.AccountKey = internal.UnquoteAll(s[1])
		case "azure_endpoint":
			table.AzureOptions.ServiceURL = internal.UnquoteAll(s[1])
		case "azure_prefix":
			table.AzureOptions.Prefix = internal.UnquoteAll(s[1])
	...
	if defaults.Prefix != "" && !prefixSpecified {
		table.AzureOptions.Prefix = joinAzurePrefix(defaults.Prefix, table.Name)
	}

	table.Tree, err = OpenKV(ctx, table.AzureOptions, "azb-rows")
	if err != nil {
		return nil, fmt.Errorf("azb: %w", err)
	}
...
func (c *VirtualTable) Insert(ctx context.Context, values map[int]interface{}) (int64, error) {
	...
	if c.usesRowID {
		...
		key = values[c.KeyCol]
		if key == nil {
			return 0, ErrAZBConstraintNotNull
		}
	}
	dbg("%T %+v\n", key, key)
	var old *v1proto.Row
	...
	if ok && (!old.Deleted || !ot.Add(old.DeleteUpdateOffset.AsDuration()).Before(t)) {
		return 0, ErrAZBConstraintPrimaryKey
	}
...
func dbg(f string, v ...interface{}) {
	slog.Debug(fmt.Sprintf(strings.TrimSpace(f), v...))
}
...
func Vacuum(ctx context.Context, tableName string, beforeTime time.Time) error {
	...
	return VacuumTable(ctx, table, beforeTime)
}

func VacuumTable(ctx context.Context, table *VirtualTable, beforeTime time.Time) error {
	...
	if err != nil {
		return fmt.Errorf("azb commit tombstones: %w", err)
	}
	_, err = db.Commit(ctx)
	if err != nil {
		return fmt.Errorf("azb commit tombstones: %w", err)
	...
	if err != nil {
		return fmt.Errorf("azb vacuum: %w", err)
	}
