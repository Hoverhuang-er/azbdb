...
usage:
 from='["version1"]',              starting version for getting changes, from azb_version()
 table='...',                      table to get changes for, must already be loaded
[to='["version2"]']                ending version for getting changes, from azb_version()`)
	}
	...
	res.table = c.sc.getTable(tableName)
	if res.table == nil {
...
func loadForDiffing(ctx context.Context, baseOptions azb.AzureOptions, versions []string) (*azb.KV, error) {
	options := baseOptions
	options.ReadOnly = true
	options.OnlyVersions = versions
	return azb.OpenKV(ctx, options, "azb-rows")
}
...
func (c *ChangesTable) Open() (sqlite.VirtualCursor, error) {
	...
	if c.fromVer == nil {
		...
		from, err = loadForDiffing(ctx, c.table.AzureOptions, c.fromVer)
		if err != nil {
			return nil, fmt.Errorf("from: %w", err)
		}
	}
	to, err := loadForDiffing(ctx, c.table.AzureOptions, c.toVer)
	if err != nil {
		return nil, fmt.Errorf("to: %w", err)
	}
...
func (c *ChangesCursor) Column(ctx *sqlite.VirtualTableContext, i int) error {
	...
	if i == c.t.KeyCol {
		...
		res = azb.FromSQLiteValue(cv.Value)
	}
