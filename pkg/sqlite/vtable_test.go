...
import (
	...
	// register azb extension with riyazali.net/sqlite
...
func openDB() (*sql.DB, string, string) {
	...
	return db, "testcontainer", sqliteAzureClient.URL()
}
...
func populateTwoTables(db *sql.DB, azureContainer, azureEndpoint, tablePrefix, schema string, row ...interface{}) (string, string, error) {
	...
	_, err = db.Exec(fmt.Sprintf(`create virtual table %s using azb (
azure_container='%s',
azure_endpoint='%s',
azure_prefix='%s',
columns='%s')`, virtualTableName,
		azureContainer, azureEndpoint, virtualTableName, schema))
	if err != nil {
...
func TODOTestEmptyText(t *testing.T) {
	db, azureContainer, azureEndpoint := getDBBucket()
	_, err := db.Exec(fmt.Sprintf(`create virtual table emptytextval using azb (
azure_container='%s',
azure_endpoint='%s',
azure_prefix='emptytextval',
columns='a')`,
...
func TODOTestEmptyBlob(t *testing.T) {
	db, azureContainer, azureEndpoint := getDBBucket()
	_, err := db.Exec(fmt.Sprintf(`create virtual table emptyblobval using azb (
azure_container='%s',
azure_endpoint='%s',
azure_prefix='emptyblobval',
columns='a')`,
		azureContainer, azureEndpoint))
	require.NoError(t, err)
...
func Test(t *testing.T) {
	openTable := func(t *testing.T, name string, db *sql.DB, azureContainer, azureEndpoint string) {
		stmt := fmt.Sprintf(`create virtual table "%s" using azb (
azure_container='%s',
azure_endpoint='%s',
azure_prefix='%s',
columns='a primary key, b')`,
			name, azureContainer, azureEndpoint, t.Name())
		_, err := db.Exec(stmt)
		require.NoError(t, err)
	}
	...
	t.Run("1", func(t *testing.T) {
		...
		defer db.Close()
		_, err := db.Exec(fmt.Sprintf(`create virtual table t1 using azb (
azure_container='%s',
azure_endpoint='%s',
azure_prefix='t1',
columns='a primary key, b')`,
	...
	t.Run("BestIndex",
		func(t *testing.T) {
			db, azureContainer, azureEndpoint := openDB()
			defer db.Close()
			regTable, sasqTable, err := populateTwoTables(db, azureContainer, azureEndpoint,
				"index", "a primary key",
				row(1),
				row(3),
				row(2),
			...
			require.NoError(t, err)
			t.Run("azbKeySorted", func(t *testing.T) {
				require.Equal(t,
					"[[1],[2],[3]]",
	...
	t.Run("SortOrder",
		...
		func(t *testing.T) {
			db, azureContainer, azureEndpoint := openDB()
			defer db.Close()
			_, err := db.Exec(fmt.Sprintf(`create virtual table sortfun using azb (
azure_container='%s',
azure_endpoint='%s',
azure_prefix='sortfun',
columns='a primary key')`,
	...
	t.Run("NullPrimaryKey",
		func(t *testing.T) {
			db, azureContainer, azureEndpoint := openDB()
			defer db.Close()
			_, err := db.Exec(fmt.Sprintf(`create virtual table nullkey using azb (
azure_container='%s',
azure_endpoint='%s',
azure_prefix='nullkey',
columns='a primary key')`,
				azureContainer, azureEndpoint))
			require.NoError(t, err)
	...
	t.Run("NullValue",
		func(t *testing.T) {
			db, azureContainer, azureEndpoint := openDB()
			_, err := db.Exec(fmt.Sprintf(`create virtual table nullval using azb (
azure_container='%s',
azure_endpoint='%s',
azure_prefix='nullval',
columns='a')`,
				azureContainer, azureEndpoint))
			require.NoError(t, err)
	...
	t.Run("ZeroInt", func(t *testing.T) {
		db, azureContainer, azureEndpoint := openDB()
		_, err := db.Exec(fmt.Sprintf(`create virtual table zerointval using azb (
azure_container='%s',
azure_endpoint='%s',
azure_prefix='zerointval',
columns='a')`,
			azureContainer, azureEndpoint))
		require.NoError(t, err)
	...
	t.Run("ZeroReal", func(t *testing.T) {
		db, azureContainer, azureEndpoint := openDB()
		_, err := db.Exec(fmt.Sprintf(`create virtual table zerorealval using azb (
azure_container='%s',
azure_endpoint='%s',
azure_prefix='zerorealval',
columns='a')`,
			azureContainer, azureEndpoint))
		require.NoError(t, err)
	...
	t.Run("InsertConflict", func(t *testing.T) {
		...
		for _, c := range cases {
			t.Run(c.expectedWinner, func(t *testing.T) {
				openTable(t, t.Name()+"1", db, azureContainer, azureEndpoint)
				openTable(t, t.Name()+"2", db, azureContainer, azureEndpoint)

				_, err := db.Exec(`update azb_conn set write_time=?`, c.firstWriteTime)
				require.NoError(t, err)
				_, err = db.Exec(fmt.Sprintf(`insert into "%s" values('row','first')`, t.Name()+"1"))
				require.NoError(t, err)
				_, err = db.Exec(`update azb_conn set write_time=?`, c.secondWriteTime)
				require.NoError(t, err)
				_, err = db.Exec(fmt.Sprintf(`insert into "%s" values('row','second')`, t.Name()+"2"))
				require.NoError(t, err)

				_, err = db.Exec(`update azb_conn set write_time=?`, "2006-01-01 00:00:00")
				require.NoError(t, err)
				openTable(t, t.Name()+"read", db, azureContainer, azureEndpoint)
	...
	t.Run("MergeValues",
		func(t *testing.T) {
			t1 := mustParseTime(azb.SQLiteTimeFormat, "2006-01-01 00:00:00")
			t2 := mustParseTime(azb.SQLiteTimeFormat, "2007-01-01 00:00:00")
			v1 := &v1proto.Row{
				ColumnValues: map[string]*v1proto.ColumnValue{"b": azb.ToColumnValue("one")},
			}
			v2 := &v1proto.Row{
				ColumnValues: map[string]*v1proto.ColumnValue{"b": azb.ToColumnValue("two")},
			}
			res := azb.MergeRows(nil, t1, v1, t2, v2, t2)
			...
			res = azb.MergeRows(nil, t2, v2, t1, v1, t1)
			...
			res = azb.MergeRows(nil, t2, v2, t1, v1, t2)
			require.Equal(t, time.Duration(0), res.DeleteUpdateOffset.AsDuration())
			require.Equal(t, time.Duration(0), res.ColumnValues["b"].UpdateOffset.AsDuration())
			res = azb.MergeRows(nil, t1, v1, t2, v2, t1)
			require.Equal(t, t2.Sub(t1), res.DeleteUpdateOffset.AsDuration())
			require.Equal(t, t2.Sub(t1), res.ColumnValues["b"].UpdateOffset.AsDuration())
		})
	...
	t.Run("Vacuum",
		func(t *testing.T) {
			...
			openTable(t, "v1", db, azureContainer, azureEndpoint)
			_, err := db.Exec(`update azb_conn set write_time=?`, "2006-01-01 00:00:00")
			require.NoError(t, err)
			...
			openTable(t, "v2", db, azureContainer, azureEndpoint)
			_, err = db.Exec(`update azb_conn set write_time=?`, "2007-01-01 00:00:00")
			require.NoError(t, err)
			_, err = db.Exec(`insert into v2 values(2007,0)`)
			require.NoError(t, err)
			require.Equal(t, `[[2007,0]]`, mustQueryToJSON(db, `select * from v2`))

			_, err = db.Exec(`update azb_conn set write_time=?`, "2008-01-01 00:00:00")
			require.NoError(t, err)
			openTable(t, "vacuumv1", db, azureContainer, azureEndpoint)
			require.Equal(t,
				`[[null]]`,
				mustQueryToJSON(db, "select * from azb_vacuum('vacuumv1','2007-01-01 00:00:00');"))
			_, err = db.Exec(`update azb_conn set write_time=?`, "2009-01-01 00:00:00")
			require.NoError(t, err)
			openTable(t, "readv2", db, azureContainer, azureEndpoint)
	...
	t.Run("NoPrefix",
		func(t *testing.T) {
			...
			_, err := db.Exec(fmt.Sprintf(`create virtual table v1noprefix using azb (
azure_container='%s',
azure_endpoint='%s',
columns='a primary key, b')`,
				azureContainer, azureEndpoint))
			require.NoError(t, err)
			_, err = db.Exec(fmt.Sprintf(`create virtual table v2noprefix using azb (
azure_container='%s',
azure_endpoint='%s',
columns='a primary key, b')`,
				azureContainer, azureEndpoint))
			...
			_, err = db.Exec(fmt.Sprintf(`create virtual table noprefix using azb (
readonly,
azure_container='%s',
azure_endpoint='%s',
columns='a primary key, b')`,
...
func TestCurrentVersion_Empty(t *testing.T) {
	...
	_, err := db.Exec(fmt.Sprintf(`create virtual table `+t.Name()+` using azb (
azure_container='%s',
azure_endpoint='%s',
azure_prefix='version_empty',
columns='a primary key, b')`,
		azureContainer, azureEndpoint))
	require.NoError(t, err)

	r, err := db.Query(`select azb_version($1)`, t.Name())
	require.NoError(t, err)
	defer r.Close()
...
func TestCurrentVersion_Multiple(t *testing.T) {
	...
	_, err := db.Exec(fmt.Sprintf(`create virtual table v1 using azb (
azure_container='%s',
azure_endpoint='%s',
azure_prefix='multiple',
columns='a primary key, b')`,
		azureContainer, azureEndpoint))
	require.NoError(t, err)
	_, err = db.Exec(fmt.Sprintf(`create virtual table v2 using azb (
azure_container='%s',
azure_endpoint='%s',
azure_prefix='multiple',
columns='a primary key, b')`,
	...
	_, err = db.Exec(fmt.Sprintf(`create virtual table merged_in_memory using azb (
readonly,
azure_container='%s',
azure_endpoint='%s',
azure_prefix='multiple',
columns='a primary key, b')`,
	...
	r, err := db.Query(`select azb_version("merged_in_memory")`)
	require.NoError(t, err)
	defer r.Close()
	require.True(t, r.Next())
	var current_version_json []byte
...
func TestDeadline(t *testing.T) {
	...
	_, err := db.Exec(fmt.Sprintf(`create virtual table deadline using azb (
azure_container='%s',
azure_endpoint='%s',
azure_prefix='deadline',
columns='a')`,
		azureContainer, azureEndpoint))
	require.NoError(t, err)

	_, err = db.Exec(`UPDATE azb_conn SET deadline='1970-01-01 00:00:00'`)
	require.NoError(t, err)
