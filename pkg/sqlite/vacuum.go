...
func (c *VacuumModule) Connect(conn *sqlite.Conn, args []string,
	...
	err := declare(`create table azb_vacuum(
		vacuum_error,
		table_name HIDDEN,
		before_time HIDDEN
	)`)
...
func (vc *VacuumCursor) Column(context *sqlite.VirtualTableContext, i int) error {
	...
	case 2:
		context.ResultText(vc.beforeTime.Format(azb.SQLiteTimeFormat))
	}
	return nil
}
...
func (vc *VacuumCursor) Filter(_ int, idxStr string, values ...sqlite.Value) error {
	...
	t, err := time.Parse(azb.SQLiteTimeFormat, values[1].Text())
	if err != nil {
		return fmt.Errorf("before_time: %w (must be like %s)", err, azb.SQLiteTimeFormat)
	}
	vc.beforeTime = t
	table := vc.module.sc.getTable(vc.tableName)
	if table == nil {
		vc.vacuumErr = fmt.Errorf("table not found: %s", vc.tableName)
		return nil
	}
	vc.vacuumErr = azb.VacuumTable(vc.module.sc.ctx, table, t)
	return nil
}
