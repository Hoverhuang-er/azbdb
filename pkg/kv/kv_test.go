...
func TestAzureBlobPersist(t *testing.T) {
	t.Parallel()
	c, containerName := newTestBlobClient(t)

	const (
		prefix = "prefix/"
		src    = "foo/123/456"
	)
	p := azurePersist{Client: c, ServiceURL: c.URL(), ContainerName: containerName, Prefix: prefix}
	...
	err = c.DeleteBlob(ctx, containerName, prefix+src)
	require.NoError(t, err)
	loaded, err = p.Load(ctx, src)
...
func TestAZBHappyCase(t *testing.T) {
	t.Parallel()
	c, containerName := newTestBlobClient(t)
	cfg := Config{
		Storage:    &AzureBlobStorageInfo{c.URL(), containerName, "happyDB"},
		KeysLike:   1234,
		ValuesLike: "hi",
	}
	s, err := Open(ctx, c, cfg, OpenOptions{}, time.Now())
...
func TestDelayedNode(t *testing.T) {
	t.Parallel()
	realC, containerName := newTestBlobClient(t)
	cfg := Config{
		Storage:    &AzureBlobStorageInfo{realC.URL(), containerName, "delayedNodeDB"},
		KeysLike:   1234,
		ValuesLike: "hi",
	}
	c := &hidingBlobClient{AzureBlobClient: realC}
	s, err := Open(ctx, c, cfg, OpenOptions{}, time.Now())
	require.NoError(t, err)
	require.Equal(t, uint64(0), s.Size())
	require.Empty(t, s.crdt.MergeSources)
	...
	c.StartHiding(sp.ContainerName, sp.Prefix)
	v1, err := s.Commit(ctx)
	require.NoError(t, err)
	roots, err := s.listRoots(ctx)
	require.NoError(t, err)
...
func dbgprintf(f string, args ...interface{}) {
	slog.Debug(fmt.Sprintf(strings.TrimSpace(f), args...))
}
...
func TestVersionGraph(t *testing.T) {
	...
	c, containerName := newTestBlobClient(t)
	cfg := Config{
		Storage:      &AzureBlobStorageInfo{c.URL(), containerName, "versionGraph"},
		KeysLike:     1234,
		ValuesLike:   "hi",
		BranchFactor: 4,
	}
	...
	allNodes, err := s.blobClient.ListBlobs(ctx, s.persist.(*persistEncryptor).ContainerName, s.persist.(*persistEncryptor).Prefix)
	require.NoError(t, err)
	nodesThatWouldBePreserved := map[string]interface{}{}
	for _, o := range allNodes {
		nodesThatWouldBePreserved[o] = nil
	}
...
func dumpRootsAndNodes(ctx context.Context, roots []string, s *DB, persist mast.Persist) {
	for i := range roots {
		slog.Debug("root nodes", slog.String("root", roots[i]), slog.Any("nodes", getNodesForRoot(ctx, s, roots[i], persist)))
	}
}

func dumpBucket(s *DB) {
	list, err := s.blobClient.ListBlobs(ctx, s.root.ContainerName, "")
	if err != nil {
		panic(err)
	}
	slog.Debug("container contents", slog.Any("blobs", list))
}

func contentHash(s *DB) string {
	return blobContentHashForPrefix(s.blobClient, s.cfg.Storage.ContainerName, s.cfg.Storage.Prefix)
}

func blobContentHashForPrefix(c AzureBlobClient, containerName, prefix string) string {
	list, err := c.ListBlobs(ctx, containerName, prefix)
	if err != nil {
		panic(err)
	}
	...
	for _, name := range list {
		...
		value, err := c.LoadBlob(ctx, containerName, name)
		if err != nil {
			panic(err)
		}
		digest.Write(value)
...
func containerUsage(c AzureBlobClient, containerName, prefix string) int {
	list, err := c.ListBlobs(ctx, containerName, prefix)
	if err != nil {
		panic(err)
	}
...
func TestAggregation(t *testing.T) {
	...
	primaryConfig := Config{
		Storage:    &AzureBlobStorageInfo{c.URL(), containerName, "primary"},
		KeysLike:   1234,
		ValuesLike: "hi",
	}
	...
	updateSecondary := func() error {
		secondary, err := Open(ctx, c, Config{
			Storage:    &AzureBlobStorageInfo{c.URL(), containerName, "secondary"},
			KeysLike:   "hi",
...
func TestDefaultNodeCacheOff(t *testing.T) {
	...
	realC, containerName := newTestBlobClient(t)
	cfg := Config{
		Storage:      &AzureBlobStorageInfo{realC.URL(), containerName, "nodeCacheOff"},
		KeysLike:     1234,
		ValuesLike:   "hi",
		BranchFactor: 4,
	}
	c := &countingBlobClient{AzureBlobClient: realC}
	s, err := Open(ctx, c, cfg, OpenOptions{}, tm.next())
	require.NoError(t, err)
...
func TestNodeCache(t *testing.T) {
	...
	realC, containerName := newTestBlobClient(t)
	cfg := Config{
		Storage:      &AzureBlobStorageInfo{realC.URL(), containerName, "nodeCacheOn"},
		KeysLike:     1234,
		ValuesLike:   "hi",
		BranchFactor: 4,
	}
...
func TestRedundantCommitDoesNotWriteToBucket(t *testing.T) {
	...
	realC, containerName := newTestBlobClient(t)
	cfg := Config{
		Storage:      &AzureBlobStorageInfo{realC.URL(), containerName, "redundantCommit"},
		KeysLike:     1234,
		ValuesLike:   "hi",
		BranchFactor: 4,
	}
...
func TestSetWithAndWithoutLaterModificationTime(t *testing.T) {
	...
	c, containerName := newTestBlobClient(t)
	cfg := Config{
		Storage:      &AzureBlobStorageInfo{c.URL(), containerName, "setSameValueWithLaterModTime"},
		KeysLike:     1234,
		ValuesLike:   "hi",
		BranchFactor: 4,
	}
...
func createTestTree(name string, key, value interface{}, keyVals ...interface{}) (*DB, AzureBlobClient, Config, TestTime, func()) {
	return createTestTreeWithConfig(name, nil, key, value, keyVals...)
}
func createTestTreeWithConfig(name string, baseCfg *Config, key, value interface{}, keyVals ...interface{}) (*DB, AzureBlobClient, Config, TestTime, func()) {
	tm := newTestTime()
	c, containerName := &memoryBlobClient{blobs: make(map[string]map[string][]byte), url: "https://test.blob.core.windows.net/"}, "testcontainer"
	var cfg Config
	if baseCfg != nil {
		cfg = *baseCfg
	}
	cfg.Storage = &AzureBlobStorageInfo{c.URL(), containerName, name}
	cfg.LogFunc = func(s string) { slog.Debug(s) }
	cfg.KeysLike = key
...
func reopen(s **DB, c AzureBlobClient, cfg Config, tm time.Time) {
	_, err := (*s).Commit(ctx)
	if err != nil {
		panic(err)
	}
...
func TestDecryptionWithWrongKey(t *testing.T) {
	...
	c, containerName := newTestBlobClient(t)
	cfg := Config{
		Storage:       &AzureBlobStorageInfo{c.URL(), containerName, "badkey"},
		KeysLike:      "stringy",
		ValuesLike:    "stringy",
		BranchFactor:  4,
...
func TestConflictDetection(t *testing.T) {
	...
	t.Run("IdempotentUpdateIsNotAConflict", func(t *testing.T) {
		...
		cfg := Config{
			OnConflictMerged: func(key, v1, v2 interface{}) error {
				slog.Debug("merged conflict", slog.Any("key", key), slog.Any("v1", v1), slog.Any("v2", v2))
				conflicts++
				return nil
			},
		}
...
func TestNodeCacheFiltersNodesCommittedByPeers(t *testing.T) {
	...
	realC, containerName := newTestBlobClient(t)
	cfg := Config{
		Storage:      &AzureBlobStorageInfo{realC.URL(), containerName, "commitClonedBlocks"},
		KeysLike:     0,
		ValuesLike:   0,
		BranchFactor: bf,
...
func TestUnmergeableBranchFactor(t *testing.T) {
	...
	c, containerName := newTestBlobClient(t)
	cfg1 := Config{
		Storage:      &AzureBlobStorageInfo{c.URL(), containerName, "unmergeable"},
		KeysLike:     0,
		ValuesLike:   0,
		BranchFactor: 16,
	}
...
func TestStatefulMarshalingEfficiency(t *testing.T) {
	...
	c, containerName := newTestBlobClient(t)
	cfg := Config{
		Storage:        &AzureBlobStorageInfo{c.URL(), containerName, "inefficient"},
		KeysLike:       1234,
		ValuesLike:     "hi",
	...
	cfg.Storage = &AzureBlobStorageInfo{c.URL(), containerName, "statefulMarshal"}
	cfg.MastNodeFormat = string(mast.V1Marshaler)
	populate(cfg)

	v115Usage := containerUsage(c, containerName, "inefficient")
	// t.Logf("with v115Binary: %d", v115Usage)
	v1Usage := containerUsage(c, containerName, "statefulMarshal")
