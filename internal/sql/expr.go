...
import (
	...
	p "github.com/Hoverhuang-er/azbdb/internal/sql/parse"
	"github.com/Hoverhuang-er/azbdb/internal/sql/types"
)

func debugSQL(format string, args ...interface{}) {
...
func toReal(cv colval.ColumnValue) *float64 {
	...
	case colval.Text:
		...
		if len(matches) > 0 {
			...
			if err != nil {
				slog.Debug("parse real", slog.Any("error", err))
			}
		}
		return &f
	}
...
func binaryExpr(res **types.Evaluator) p.Func {
	...
	return func(e *p.Parser) bool {
		...
		for {
			...
			if e.Exact("(") {
				...
				valStack = append(valStack, types.Evaluator{
					Func: func(inputs map[string]colval.ColumnValue) colval.ColumnValue {
						debugSQL("deref %s -> %v", name, inputs[name])
						res, ok := inputs[name]
						if !ok {
			...
			e.SkipWS()
			for {
				debugSQL("input: %s", e.Remaining)
				if false {
					debugSQL("valStack: %v", valStack)
				}
				debugSQL("opStack: %v", opStack)
				debugSQL("precStack: %v", precStack)
