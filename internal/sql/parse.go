	...
	return parse.OneOf(
		...
		parse.RE(RealValueRE, func(s []string) bool {
			if !strings.ContainsAny(s[0], ".eE") {
				debugSQL("parsing as int")
				i, err := strconv.ParseInt(s[0], 0, 64)
				debugSQL("parsed int: %d, conversion error: %v", i, err)
				if err == nil {
					*cv = colval.Int(i)
					return true
				}
...
func debugHere(s string) parse.Func {
	return func(p *parse.Parser) bool {
		debugSQL("debugHere %s, remaining: %s", s, p.Remaining)
		return true
	}
}
...
func FindColumnIndex(s *types.Schema, name string) *int {
	for i, c := range s.Columns {
		if strings.EqualFold(c.Name, name) {
			debugSQL("returning matched column %d, c.Name=%s, name=%s", i, c.Name, name)
			return &i
		}
	}
	return nil
...
func FromItemAs(as *string) parse.Func {
	return parse.SeqWS(
		parse.Exact("").Action(func() { debugSQL("starting fromAs") }),
		parse.Optional(parse.CI("as")),
		FromItemAlias(as))
}
