...
func Test(t *testing.T, opener DBOpener) {
	...
	t.Run("1", func(t *testing.T) {
		slog.Debug("opening test database")
		db, azureContainer, azureEndpoint := opener.OpenDB()
		slog.Debug("deferring test database close")
		defer db.Close()
		slog.Debug("executing virtual table setup")
...
columns='a')`,
	...
	t.Run("InsertConflict", func(t *testing.T) {
		...
		openTableWithWriteTime := func(t *testing.T, name, tm string) {
			mustParseTime(azb.SQLiteTimeFormat, tm)
...
columns='a primary key, b',
write_time='%s')`,
				name, azureContainer, azureEndpoint, t.Name(), tm)
			_, err := db.Exec(stmt)
			require.NoError(t, err)
		}
