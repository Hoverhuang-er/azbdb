	...
	if o == nil && n == nil {
		...
		if o.considerLink != nil && n.considerLink != nil {
			if o.considerLink != n.considerLink {
				if m.debug {
					debugf("  old(consider) new(consider) and links differ\n")
				}
				if !dc.oldMast.alreadyNotified(ctx, "old", dc.alreadyNotifiedOldLink, o.considerLink) {
					dc.removedLink = o.considerLink
				}
				...
				if len(oldNode.Link) == 1 {
					...
					dc.newStack.push(n)
					if m.debug {
						debugf("  oldStack descending through empty intermediate\n")
					}
					return nil
				}
				...
				if len(newNode.Link) == 1 {
					...
					if m.debug {
						debugf("  newStack descending through empty intermediate\n")
					}
					return nil
				}
				...
				if m.debug {
					debugf("  oldKey=%v.compare(newKey=%v): %d\n", oldKey, newKey, cmp)
				}
				if cmp < 0 {
					dc.oldStack.pushNode(oldNode)
					dc.newStack.push(n)
