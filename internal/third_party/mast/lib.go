...
func split(ctx context.Context, node *mastNode, key interface{}, mast *Mast) (leftLink, rightLink interface{}, err error) {
	...
	if leftMaxLink != nil {
		...
		if mast.debug {
			debugf("  splitting leftMax, node with keys: %v\n", leftMax.Key)
		}
		if err != nil {
		...
		if mast.debug {
			debugf("  splitting leftMax, node with keys: %v is done: leftMaxLink=%v, tooBigLink=%v\n", leftMax.Key, leftMaxLink, tooBigLink)
		}
	...
	if rightMinLink != nil {
		...
		if mast.debug {
			debugf("  splitting rightMin, node with keys %v, is done: tooSmallLink=%v, rightMinLink=%v",
...
func (m *Mast) shrink(ctx context.Context) error {
	...
	if m.debug {
		debugf("SHRINKING\n")
		debugf("size=%d height=%d branchFactor=%d\n", m.size, m.height, m.branchFactor)
		m.dump(ctx)
	}
	if m.height == 0 {
...
func (node *mastNode) iter(ctx context.Context, f func(interface{}, interface{}) error, mast *Mast) error {
	if mast.debug {
		debugf("starting iter at node with keys: %v\n", node.Key)
	}
